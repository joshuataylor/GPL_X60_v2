...
{
    char *product_name = getenv(ENV_STR_PRODUCT_NAME);
    char *hardware_verison_str = getenv(ENV_STR_PRODUCT_VERSION);
    char *special_id_str = getenv(ENV_STR_SPECIAL_ID);
    char *vendor_name = getenv(ENV_STR_VENDOR_NAME);
    char *product_language = getenv(ENV_STR_PRODUCT_LANGUAGE);
    char *vendor_url = getenv(ENV_STR_VENDOR_URL);
    char *product_id_str = getenv(ENV_STR_PRODUCT_ID);
