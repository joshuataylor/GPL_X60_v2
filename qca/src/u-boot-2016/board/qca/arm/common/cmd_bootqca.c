...
#define nand_rootfs_1 "ubi.mtd=" QCA_ROOT_FS_ALT_PART_NAME " root=mtd:ubi_rootfs rootfstype=squashfs"
	...
	if (sfi->flash_type == SMEM_BOOT_SPI_FLASH) {
		if (get_which_flash_param("rootfs") ||
			...
			*fs_on_nand = 1;
			...
			if (sfi->rootfs[idx].offset == 0xBAD0FF5E) {
				...
				sfi->rootfs[idx].offset = active_part * IPQ_NAND_ROOTFS_SIZE;
		...
		bootargs = idx ? nand_rootfs_1 : nand_rootfs;
		if (getenv("fsbootargs") == NULL)
			setenv("fsbootargs", bootargs);
		*fs_on_nand = 1;
                             ...
                             int idx)
{
	...
	if (ipq_fs_on_nand) {
		...
		kernel_img_info.kernel_load_size =
			...
			((sfi->flash_type == SMEM_BOOT_SPI_FLASH) &&
			(sfi->rootfs[idx].offset == 0xBAD0FF5E))) {
		blk_dev = mmc_get_dev(host->dev_num);
		if (smem_bootconfig_info() == 0) {
			active_part = get_rootfs_active_partition();
			if (active_part) {
...
#define LED_GPIO_R      31
#define LED_GPIO_G      30
#define LED_GPIO_B      32

static void set_fw_led(int state)
{
        switch (state)
        {
        case FW_LED_START:
        /* Turn green */
                gpio_set_value(LED_GPIO_R, 0);
                gpio_set_value(LED_GPIO_G, 1);
                gpio_set_value(LED_GPIO_B, 0);
                return;
        case FW_LED_HTTPD:
                gpio_set_value(LED_GPIO_R, 0);
                gpio_set_value(LED_GPIO_G, 0);
                gpio_set_value(LED_GPIO_B, 0);
                mdelay(1000);
                gpio_set_value(LED_GPIO_G, 1);
                mdelay(1000);
