...
#define LED_R_GPIO 31
#define LED_G_GPIO 30
#define LED_B_GPIO 32
...
static int is_test_mode(void)
{
	...
	char runcmd[256];
	char *boot_idx = getenv(BOOT_IDX_FLAG);

	if (smem->flash_type == SMEM_BOOT_NAND_FLASH) {
		/*
		 * qca part entries are initialized in board_late_init, 
		 * so put board_led_init after board_late_init.
		 */		
		snprintf(runcmd, sizeof(runcmd),
			 "setenv mtdids nand0=nand0 && "
			 "setenv mtdparts mtdparts=nand0:0x%llx@0x%llx(fdata),${msmparts} && "
			 "ubi part fdata && "
			 "ubifsmount ubi:ubi_factory_data",
			 smem->f_data.size, smem->f_data.offset);

		/*printf("runcmd is %s\n", runcmd);*/

		if (run_command(runcmd, 0) != CMD_RET_SUCCESS) {
			printf("fail to mount ubi_factory_data\n");
			return 1;
		}

		/* check which image to boot */
		if (boot_idx && (strcmp(boot_idx, "1") == 0)) {
			printf("Find boot alter flag!\n");
			has_boot_alter_flag = 1;
		} else {
			printf("Find no boot alter flag!\n");
			has_boot_alter_flag = 0;
		}

#ifdef CONFIG_CMD_UBIFS
		if (!ubifs_exists("/default-mac")) {
			printf("/default-mac doesn't exist!\n");
			return 1;
		}
#endif
	}
...
void board_led_init(void)
{
	...
	gpio_node = fdt_path_offset(gd->fdt_blob, "/leds/led_gpio");
	if (gpio_node >= 0) {
		qca_gpio_init(gpio_node);
	}
