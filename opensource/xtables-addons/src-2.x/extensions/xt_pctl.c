...
#if defined(SUPPORT_SHORTCUT_FE) || defined(CONFIG_TP_FC_PCTL_SUPPORT)
                       ...
                       int status, unsigned int stamp)
{
    ...
    pDnsHdr = (dns_header *)((unsigned char *) udph + sizeof(struct udphdr));
...
{   
...
#if defined(SUPPORT_SHORTCUT_FE) || defined(CONFIG_TP_FC_PCTL_SUPPORT) || defined(CONFIG_LTQ_PPA_API_SW_FASTPATH)
		struct nf_conn *ct;
		enum ip_conntrack_info ctinfo;
#endif
	...
	ct = nf_ct_get(skb, &ctinfo);
#if !defined(CONFIG_TP_FC_PCTL_SUPPORT) && !defined(CONFIG_LTQ_PPA_API_SW_FASTPATH)
	if (ct) {
...
#if GET_BR_LAN_IP
#if (LINUX_VERSION_CODE > KERNEL_VERSION(3,10,13))
       lan_ip_proc_file = proc_create(LAN_IP_PROC_FILENAME, S_IRUGO, proc_dir, &lan_ip_proc_fops);
       if(!lan_ip_proc_file) 
       {
           PCTL_ERROR("proc_create %s failed.", LAN_IP_PROC_FILENAME);
           return -1;
       }
#else
...
#if URL_LIB
#if (LINUX_VERSION_CODE > KERNEL_VERSION(3,10,13))
       url_lib_proc_file = proc_create(URL_LIB_PROC_FILENAME, S_IRUGO, proc_dir, &url_lib_proc_fops);
       if(!url_lib_proc_file) 
       {
           PCTL_ERROR("proc_create %s failed.", URL_LIB_PROC_FILENAME);
           return -1;
       }
#else
