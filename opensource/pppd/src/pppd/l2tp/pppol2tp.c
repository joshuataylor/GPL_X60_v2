...
static const char pppd_version[] = VERSION;
...
static int connect_pppol2tp(void)
{
	...
	if (nic_ifname[0])
	{
		setsockopt(pppol2tp_fd, SOL_SOCKET, SO_BINDTODEVICE, nic_ifname, strlen(nic_ifname) + 1);
	}
...
/* changed name from plugin_init to pppol2tp_init  */
int pppol2tp_init(void)
{
